...
/**
 * MessageHandler implementations must deal with the different kinds of incoming messages (sent by the Coordinator).
 * The implementations should react to these messages either by modifying the context, calling external handlers,
 * logging received information, respond to the messages if necessary, etc. Since the agents by nature work in an
 * asynchronous manner, all message handler implementation must adhere that.
 *
...
export interface MessageHandler<T> {
    ...
    /**
     * Executes the message processing logic.
     *
